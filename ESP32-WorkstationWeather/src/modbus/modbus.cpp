...
bool modbusInitialization(){
  ...
  if(!ModbusRTUClient.begin(modbusBaudRate, SERIAL_8N1)){   //Modbus client configured as 8 data bits, Even parity and 1 stop bit (SERIAL_8E1).
    DEBUG_PRINT Serial.println("Modbus Client not initialized!");
    return false; 
  }
  else {
    DEBUG_PRINT Serial.println("Modbus Client successfully initialized");
    return true;
  }
...
float modbusTemperature(){
  float temperature = float(ModbusRTUClient.inputRegisterRead(slaveAdress, temperatureRegister) / 10.00F);
  // DEBUG_PRINT Serial.printf("Temperature: %.2f \t",temperature);
  return temperature;
}

float modbusHumidity(){
  float humidity = float(ModbusRTUClient.inputRegisterRead(slaveAdress, humidityRegister) / 10.00F);
  // DEBUG_PRINT Serial.printf("Humidity : %.2f \r\n",humidity);
  return humidity;
}
